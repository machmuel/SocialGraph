...
if (-not $page) { throw 'SocialGraph tab not found.' }
...
function Send-Cdp([string]$method, $params = $null) {
  ...
  $null = $script:ws.SendAsync([ArraySegment[byte]]::new($bytes), [Net.WebSockets.WebSocketMessageType]::Text, $true, [Threading.CancellationToken]::None).GetAwaiter().GetResult()
  return $script:id
}
function Receive-Cdp([int]$timeoutMs = 5000) {
  ...
  try {
    do {
      $res = $script:ws.ReceiveAsync([ArraySegment[byte]]::new($buffer), $cts.Token).GetAwaiter().GetResult()
...
while ([DateTime]::UtcNow -lt $deadline) { $msg = Receive-Cdp 500; if ($null -ne $msg) { $events.Add($msg) | Out-Null } }
...
$null = $ws.CloseAsync([Net.WebSockets.WebSocketCloseStatus]::NormalClosure, 'done', [Threading.CancellationToken]::None).GetAwaiter().GetResult()
...
[pscustomobject]@{
  page = $summary.result.result.value
